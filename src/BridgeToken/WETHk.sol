 ...
 * @notice Bridge token representing Wrapped ETH on non-native chains
 * @dev ERC20 token with mint/burn functionality for cross-chain operations
 * Uses 18 decimals to match WETH standard
 */
contract WETHk is ERC20, Ownable {
    ...
    /**
     * @notice Constructor to initialize the WETH representative token
     * @dev Sets up the token with name, symbol, and owner
     */
    constructor() ERC20("Wrapped ETH representative", "WETHk") Ownable(msg.sender) {}
     ...
     * @return The number of decimals (18, matching WETH)
