 ...
 * @notice Bridge token representing USDC on non-native chains
 * @dev ERC20 token with mint/burn functionality for cross-chain operations
 * Uses 6 decimals to match USDC standard
 */
contract USDCk is ERC20, Ownable {
    ...
    /**
     * @notice Constructor to initialize the USDC representative token
     * @dev Sets up the token with name, symbol, and owner
     */
    constructor() ERC20("USD Circle representative", "USDCk") Ownable(msg.sender) {}
     ...
     * @return The number of decimals (6, matching USDC)
